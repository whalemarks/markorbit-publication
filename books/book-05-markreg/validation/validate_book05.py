...
def check_inventory() -> None:
    ...
    for p in manuscripts:
        ...
        metadata_ok = "**Status:** Complete Draft 1" in text and "**Chapter Map:** B05-TOC-V0.1 — Owner Accepted" in text
        record(f"chapter {num:02d} heading", title_ok, first_heading(text) or "missing heading")
        record(f"chapter {num:02d} metadata", metadata_ok, "required metadata present" if metadata_ok else "missing required metadata")
    ...
    for p in markdown_files():
        ...
        for m in re.finditer(r"^#{1,6}\s+(.+?)\s*$", text, re.M):
            ...
            if s in seen:
                duplicates.add(s)
            seen.add(s)
        record(f"headings {p.relative_to(BOOK)}", not duplicates, "unique" if not duplicates else f"duplicates: {sorted(duplicates)}")
        ...
        for target in link_re.findall(text):
            ...
            dest = (p.parent / unquote(raw_path)).resolve()
            try:
...
def check_controlled_ids() -> None:
    ...
    for kind, number in re.findall(r"\bMR-(CR|A|C|D|E|B|V|G|SCN)-(\d{2})\b", corpus):
        n = int(number)
...
def check_figures_source() -> list[Path]:
    ...
    for p in figures:
        ...
        for label, tokens in {
            ...
            "grayscale": ["grayscale", "greyscale"], "authority": ["authority boundary"],
        }.items():
...
def check_yaml_state() -> None:
    ...
    record("YAML figure planned count", apparatus.get("figures_planned") == 12, str(apparatus.get("figures_planned")))
    record("YAML figure retained count", apparatus.get("figures_retained") == 11, str(apparatus.get("figures_retained")))
...
def render_figures(figures: list[Path]) -> dict[str, Path]:
    ...
    for p in figures:
        ...
        cp = run_command([mmdc, "-i", str(source), "-o", str(svg), "-c", str(config), "-b", "transparent"])
...
def strip_metadata(text: str) -> str:
    return "\n".join(line for line in text.splitlines() if not re.match(r"^\*\*(Status|Chapter Map|Part):\*\*", line))
...
def build_reader(rendered: dict[str, Path]) -> tuple[Path, Path]:
    ...
    for fig_id, svg in sorted(rendered.items()):
        ...
        figure_parts.append(f'<figure id="{fig_id.lower()}"><h2>{html.escape(title)}</h2><img src="{rel}" alt="{html.escape(caption)}"><figcaption>{html.escape(caption)}</figcaption></figure>')

    html_path = BUILD / "book-05-reader.html"
    pdf_path = BUILD / "book-05-reader.pdf"
    document = f"""<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Book 05 — MarkReg</title><style>{css}</style></head>
<body><header><h1>Book 05 — MarkReg: The Full-Lifecycle International Trademark Product</h1><p>PF-08 validation edition. Complete Draft 1; not Release Candidate 1.</p></header>
<nav><h2>Contents</h2><ol>{''.join(toc)}</ol></nav>{''.join(sections)}
<section class="figure-atlas"><h1>Controlled Figure Atlas</h1>{''.join(figure_parts)}</section></body></html>"""
...
def validate_rendered(pdf_path: Path, rendered: dict[str, Path]) -> None:
    ...
    missing_titles = [first_heading(read(p)) for p in reader_sources() if first_heading(read(p)) not in all_text]
    record("rendered document completeness", not missing_titles, "all source document titles present" if not missing_titles else f"missing {missing_titles[:10]}")


def write_report() -> None:
    BUILD.mkdir(parents=True, exist_ok=True)
    summary = {"status": "PASS" if not ERRORS else "FAIL", "checks": len(CHECKS), "passed": sum(c.status == "PASS" for c in CHECKS), "warnings": WARNINGS, "errors": ERRORS, "results": [asdict(c) for c in CHECKS]}
...
def main() -> int:
    ...
    if not args.skip_render:
        ...
        _, pdf_path = build_reader(rendered)
        validate_rendered(pdf_path, rendered)
    write_report()
